# 🛡️ SISTEMA DE BITÁCORAS DIARIAS DE DESARROLLO — UPROTA

Este directorio contiene las bitácoras oficiales de desarrollo del equipo de UPROTA. Cada jornada de trabajo cuenta con un archivo independiente en formato Markdown con el nombre: `YYYY-MM-DD_Bitacora_Dia_XX.md`.

---

## 🏛️ ESTRUCTURA ESTÁNDAR DE CADA BITÁCORA:

Cada documento diario se divide en 4 secciones obligatorias:

1. **🌟 RESUMEN GENERAL Y ESTADO DE PRODUCCIÓN:** Hito principal alcanzado en la jornada.
2. **🔨 REGISTRO DE APORTES POR INTEGRANTE:**
   - 👑 **Director (Anigami Agadni):** Directivas, decisiones de diseño y feedback.
   - ⚡ **Nexo (Ingeniería Principal):** Código integrado, lógica, optimizaciones y despliegues.
   - 🎨 **Pix (Arte Pixel):** Sprites, animaciones, dioramas y paletas de color.
   - 📜 **Silas (Lore y Narrativa):** Guiones de radio, eventos, psicología de hábitos y datasets.
   - 🎧 **Hertz (Sonido y Chiptune):** Síntesis procedural, SFX, trackers y temas musicales.
   - 📡 **Éter (Estratega de Difusión):** Enlaces transmedia, branding y relaciones comunitarias.
3. **📬 TABLÓN DE MENSAJES Y ENCARGOS CRUZADOS:** Mensajes directos de un miembro para otro.
4. **🎯 PRÓXIMOS PASOS / ROADMAP INMEDIATO:** Metas para la siguiente jornada.

---

## 📜 REGLA DE ORO PARA EL EQUIPO:
Cada vez que un compañero complete una tarea, añada código, exporte sprites o redacte lore, debe **abrir la bitácora del día actual y registrar su avance y sus mensajes para los demás**.
**No borrar los mensajes; escribir en las líneas siguientes (append-only). Debe quedar un historial fidedigno de cómo fluyó el trabajo.**

---

## 📚 REGISTRO DE BITÁCORAS ACTIVAS:
- [Jornada 01 (30 de Agosto de 2026)](file:///c:/Users/Snow/.gemini/antigravity/scratch/UPROTA/docs/bitacoras/2026-08-30_Bitacora_Dia_01.md)
- [Jornada 02 (09 de Septiembre de 2026)](file:///c:/Users/Snow/.gemini/antigravity/scratch/UPROTA/docs/bitacoras/2026-09-09_Bitacora_Dia_02.md)
- [Jornada 03 (13 de Septiembre de 2026)](file:///c:/Users/Snow/.gemini/antigravity/scratch/UPROTA/docs/bitacoras/2026-09-13_Bitacora_Dia_03.md)
- [Jornada 04 (17 de Septiembre de 2026)](file:///c:/Users/Snow/.gemini/antigravity/scratch/UPROTA/docs/bitacoras/2026-09-17_Bitacora_Dia_04.md)
